...
them. The analysis document is the binding authority, the plan is its
...
operation; a security-sensitive action; a side effect outside this
repository that norms say you ask about first (a merge, a push to a shared
branch, a publish); and a plan so broken that every path forward is a
...
**Several plan files under one index?** Use cf-powers:orchestrator instead — it
delegates each phase (to a whole-phase executor, or to this skill for the big
...
digraph process {
    ...
    "Rule and continue; stop only if every path forward is a guess" -> "Park findings in ledger with rulings";
    ...
    "Final review clean: delete this plan's workspace" -> "Use cf-powers:finishing-a-development-branch";
}
```
...
todo per task. If the plan names an **Analysis** (cf-powers:writing-plans puts
that pointer in the plan header), read it too: the analysis is the authority
the plan argues from, and conflicts inside the plan resolve against it. A plan
with no reachable analysis gets a ledger note saying so — rulings made without
one are provisional.
...
Write the table to the ledger. Rule on each conflict it surfaces — the
analysis is the binding authority, the plan is its argument — record the
ruling beside its row, and dispatch Task 1. If the scan is clean, proceed
...
judgement, its own tests, or its own review surface.
...
timeouts, and never sit in one silent, open-ended wait either. While you
have local work — ledger updates, packaging the next review, reading
reports — keep working; child results arrive on their own. When you are
genuinely idle, wait in bounded stretches (five to ten minutes, where your
platform allows), and between stretches post one line of status and
...
- The dispatch carries the no-subagents contract (it is in the implementer
  ...
  never a reviewer. Review arrives from you, after the report. In real
  sessions, every reviewer a worker spawned duplicated the task review the
  controller dispatched anyway — a full extra review seat per task.
...
- A finding labeled plan-mandated — or any finding that conflicts with
  ...
  against the plan text, decide with the analysis as the binding authority,
...
| "This conflict needs my human partner's call" | Only four things stop you. Everything else is a `Ruling:` in the ledger, and you keep going. |
| "Six small one-line fixes, six dispatches" | Same-shape micro-edits batch into one dispatch and one review. Six seats for six one-liners is pure overhead. |
...
Done! Using cf-powers:finishing-a-development-branch.
```
