...
if echo "$output" | grep -qi "superpowers:analysis\|superpowers:using-superpowers\|Available skills"; then
...
echo "  Running opencode with superpowers:analysis skill..."

output=$(timeout 60s opencode run --print-logs "Use the use_skill tool to load superpowers:analysis and tell me the first few lines of what you received." 2>&1) || {
    exit_code=$?
    if [ $exit_code -eq 124 ]; then
        echo "  [FAIL] OpenCode timed out after 60s"
        exit 1
...
# Check for expected content from analysis skill
if echo "$output" | grep -qi "analysis\|Launching skill\|skill.*loaded"; then
    echo "  [PASS] use_skill loaded superpowers:analysis skill"
else
    echo "  [FAIL] use_skill did not load superpowers:analysis correctly"
    echo "  Output was:"
    echo "$output" | head -50
    exit 1
fi
